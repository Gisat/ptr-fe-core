...
import "../map.css";
import { defaultMapState, defaultMapView } from "../_logic/map.defaults";
import React, { useState } from "react";
import { parseLayersFromSharedState } from "../_logic/parsing.layers";

import FeatureDetailModal, { FeatureModalState } from "./FeatureDetailModal";
import { FeatureInteractionFunc } from "../_logic/models.events";
import { useSharedState } from "../../(shared)/_hooks/state.useSharedState";
...
export const RenderingMap: React.FC<RenderMapProps> = (props: RenderMapProps) => {
	...
	interactionMap.set(
		LayerTreeInteraction.FeatureDetail,
		(propertiesEntity: Nullable<any>) => {

			if (!propertiesEntity)
				throw new Error(`Feature Interaction: Callback for ${LayerTreeInteraction.FeatureDetail} requires feature property data`);
		})

	// deck GL map view state
	const initMapState: MapViewState = defaultMapState()
