 ...
 * @param datasource Data source node 
...
export const validateDatasource = (
  ...
  if (!configuration)
    throw new Error(`Datasource error: ${requiredDatasourceType} requires congfiguration`);

  const configurationJs = typeof configuration === "string"
      ? JSON.parse(configuration)
      : configuration;


  if (!key)
    throw new Error(`Datasource error: ${requiredDatasourceType} requires key`);
