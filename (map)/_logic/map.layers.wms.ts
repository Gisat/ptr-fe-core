 ...
 */
export const createWMSLayer = ({sourceNode, isActive}: LayerGeneralProps) => {

  const {configurationJs} = validateDatasource(sourceNode, UsedDatasourceLabels.WMS, true)

  if (!configurationJs.sublayers)
    throw new Error(`WMS Layer error: WMS requires sublayers in configuration`)

  const { configuration, key } = sourceNode
