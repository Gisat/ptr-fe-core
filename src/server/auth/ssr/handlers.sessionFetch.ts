...
export const fetchWithSessions = async (props: FetchWithBrowserSessionProps): Promise<FetchWithSessionsResponse> => {
	...
	if (!url)
		throw new BaseHttpError('Missing URL for session fetch', HttpStatusCode.INTERNAL_SERVER_ERROR, ErrorBehavior.SSR);
	...
	if (!sessionCookie && props.requireSessionId)
		throw new BaseHttpError('Missing session from browser', HttpStatusCode.UNAUTHORIZED, ErrorBehavior.BE);
