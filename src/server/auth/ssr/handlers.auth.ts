...
export function authContext(clientId: Unsure<string>, issuerUrl: Unsure<string>, redirectUrl: Unsure<string>) {
	...
	const checkContextEnvironmens = () => {
		if (!issuerUrl)
			throw new BaseHttpError('Missing OID issuer URL', HttpStatusCode.INTERNAL_SERVER_ERROR, ErrorBehavior.SSR);

		if (!clientId)
			throw new BaseHttpError('Missing OID client ID', HttpStatusCode.INTERNAL_SERVER_ERROR, ErrorBehavior.SSR);
	...
	async function handleLogout(tokenExchangeUrl: Unsure<string>) {
		if (!tokenExchangeUrl)
			throw new BaseHttpError('Missing URL for exchange', HttpStatusCode.INTERNAL_SERVER_ERROR, ErrorBehavior.SSR);
	...
	async function handleAuthCallback(params: any, tokenExchangeUrl: Unsure<string>) {
		...
		if (!tokens.access_token)
			throw new BaseHttpError('Missing OID information', HttpStatusCode.INTERNAL_SERVER_ERROR, ErrorBehavior.SSR);
		...
		if (!tokens.refresh_token)
			throw new BaseHttpError('Missing OID information', HttpStatusCode.INTERNAL_SERVER_ERROR, ErrorBehavior.SSR);
		if (!clientId)
			throw new BaseHttpError('Missing client ID', HttpStatusCode.INTERNAL_SERVER_ERROR, ErrorBehavior.SSR);
		if (!tokenExchangeUrl)
			throw new BaseHttpError('Missing URL for excgange', HttpStatusCode.INTERNAL_SERVER_ERROR, ErrorBehavior.SSR);
		if (!issuerUrl)
			throw new BaseHttpError('Missing issuer URL', HttpStatusCode.INTERNAL_SERVER_ERROR, ErrorBehavior.SSR);
