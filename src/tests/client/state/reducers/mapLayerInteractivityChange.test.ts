...
describe('Reducer test: Map layer interactivity change', () => {
	...
	it('throws when payload is missing', () => {
		const state = { ...fullAppSharedStateMock };
		const invalidAction = { type: StateActionType.MAP_LAYER_INTERACTIVITY_CHANGE } as unknown as ActionMapLayerInteractivityChange;
		expect(() => reduceHandlerMapLayerInteractivityChange(state, invalidAction)).toThrow(
